#include "headers/delocate_memory.h"

using namespace std;

/**
 * Free a single linear 3D array
 *
 * @param arr Pointer to linear array to free
 */
void delocate_memory_linear(double *arr) {
    if (arr != NULL) {
        free(arr);
    }
}

/**
 * Free all four main arrays (c, c_n, f, f_n)
 *
 * @param f    Pointer to f array
 * @param f_n  Pointer to f_n array
 * @param c    Pointer to c array
 * @param c_n  Pointer to c_n array
 * @param nx   (unused, kept for compatibility)
 * @param ny   (unused, kept for compatibility)
 * @param nz   (unused, kept for compatibility)
 */
void delocate_memory(
    double *f, double *f_n, double *c, double *c_n,
    int nx, int ny, int nz) {

    // With linear arrays, deallocation is trivial!
    delocate_memory_linear(f);
    delocate_memory_linear(f_n);
    delocate_memory_linear(c);
    delocate_memory_linear(c_n);

    printf("Memory deallocated successfully\n");
}
