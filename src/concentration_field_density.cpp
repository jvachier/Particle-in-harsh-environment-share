	...
	for (int i = 1; i < nx - 1; i++)
	{
		for (int j = 1; j < ny - 1; j++)
		{
			for (int k = 1; k < nz - 1; k++)
			{
				...
				f[i][j][k] = f_n[i][j][k] + beta * (Fx * u_x * Cx * c_x + Fy * u_y * Cy * c_y + Fz * u_z * Cz * c_z + f_n[i][j][k] * (Cxx * c_xx + Cyy * c_yy + Czz * c_zz)) + Fz * advection[k] * u_z - dt * reaction[k] * f_n[i][j][k] + diffusion[k] * (Fxx * u_xx + Fyy * u_yy + Fzz * u_zz);
			}
		}
	}
}
